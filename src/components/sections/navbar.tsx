...
import { Search, Phone, Mail, Instagram, Twitter, ChevronDown, Menu, X, Settings } from "lucide-react";
...
const Navbar = () => {
  ...
  const navLinks: NavLink[] = [
    ...
    {
      name: "Departments",
      href: "/departments/central-administration",
      dropdown: [
        { name: "Central Administration", href: "/departments/central-administration" },
        { name: "Finance", href: "/departments/finance" },
        { name: "Education, Youth & Sports", href: "/departments/education" },
        { name: "Health", href: "/departments/health" },
        { name: "Agriculture", href: "/departments/agriculture" },
        { name: "Social Welfare", href: "/departments/social-welfare" },
        { name: "Works", href: "/departments/works" },
  ...
  return (
    <header className="w-full font-sans">
      <div className="bg-[#8B0000] text-white py-[10px] hidden md:block border-b border-[#6B0000]">
        <div className="container mx-auto px-4 flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-0 text-[12px] sm:text-[13px] font-medium">
          ...
          <div className="flex items-center space-x-4">
            <a href="#" className="hover:text-[#ffcc00] transition-colors" aria-label="Facebook">
            ...
            <a href="#" className="hover:text-[#ffcc00] transition-colors" aria-label="Twitter">
              <Twitter className="w-4 h-4 fill-current" />
            </a>
            <a href="#" className="hover:text-[#ffcc00] transition-colors" aria-label="Instagram">
      ...
      <nav
        className={`w-full bg-white z-[1000] border-b border-gray-100 transition-all duration-300 ${
          isSticky
          ...
          <div className="flex-shrink-0">
            <Link href="/" className="inline-flex items-center gap-2 sm:gap-3 py-2">
              ...
              <div className="flex flex-col">
                <span className="text-[#8B0000] font-bold text-xs sm:text-sm leading-tight">GA SOUTH</span>
            ...
            <ul className="flex items-center list-none m-0 p-0">
              {navLinks.map((link) => (
                <li key={link.name} className="relative group px-1">
                  <Link
                    href={link.href}
                    className="flex items-center px-2 xl:px-3 py-2 text-[12px] xl:text-[13px] font-bold text-[#333333] uppercase tracking-wide hover:text-[#8B0000] transition-colors group-hover:text-[#8B0000]"
                  >
                    {link.name}
                    {link.dropdown && (
                  ...
                  {link.dropdown && (
                    <div className="absolute top-full left-0 w-56 bg-white border-t-2 border-[#8B0000] shadow-xl opacity-0 invisible translate-y-2 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-300 z-50">
                      <ul className="py-2">
                        {link.dropdown.map((subItem) => (
                          <li key={subItem.href}>
                            <Link
                              href={subItem.href}
                              className="block px-5 py-2 text-[12px] font-medium text-gray-700 hover:bg-gray-50 hover:text-[#8B0000] transition-colors border-b border-gray-50 last:border-0"
                            >
                              {subItem.name}
                            </Link>
                          </li>
...
<button className="ml-2 xl:ml-4 p-2 text-[#333333] hover:text-[#8B0000] transition-colors cursor-pointer" aria-label="Search">
              <Search className="w-5 h-5 stroke-[2.5]" />
            </button>
            <Link href="/admin/login" className="ml-1 p-2 text-[#333333] hover:text-[#8B0000] transition-colors" aria-label="Admin">
          ...
          <div className="overflow-y-auto h-[calc(100vh-60px)] pb-20">
            ...
            <div className="px-4 sm:px-6 py-6 sm:py-8 bg-gray-50 border-t flex flex-col gap-4">
              ...
              <div className="flex gap-3 pt-2">
                <a href="#" className="p-2 bg-white rounded hover:bg-gray-100 transition-colors" aria-label="Facebook">
                  <svg className="w-4 h-4 fill-[#8B0000]" viewBox="0 0 24 24">
                    <path d="M18.77,7.46H14.5v-1.9c0-.9.6-1.1,1-1.1h3V.53L14.17.53A5.44,5.44,0,0,0,8.44,6v1.46H5v4.59H8.44V23.47h6.06V12.05h4.1l.17-4.59Z" />
                  </svg>
                </a>
                <a href="#" className="p-2 bg-white rounded hover:bg-gray-100 transition-colors" aria-label="Twitter">
                  <Twitter className="w-4 h-4 text-[#8B0000]" />
                </a>
                <a href="#" className="p-2 bg-white rounded hover:bg-gray-100 transition-colors" aria-label="Instagram">
